...
Traffic-simulation harness for LLM inference servers (built for
[TokenSpeed](https://github.com/lightseekorg/tokenspeed), works against any
OpenAI-compatible `/v1/chat/completions` endpoint). Emits structured JSONL
events and produces a summary with TTFT/TPOT/E2E percentiles, error taxonomy,
...
```bash
pip install -e .            # installs the only hard dep, aiohttp
pip install jsonschema      # optional: enables JSON-Schema validation in grammar workloads
```

Run it as a module from the repo root: `python -m llm_stress ...`.
...
- Python 3.9+ and `aiohttp` (installed by `pip install -e .`).
...
```
llm_stress/
  __main__.py         # CLI: `python -m llm_stress run ...`
...
```bash
DURATION_S=120 bash llm_stress/scripts/kimi-k2.5-nvfp4-reality-mix.sh
```

The `Stress Test` GitHub workflow (`.github/workflows/stress.yml`) is
config-agnostic: it just installs the harness and runs the chosen scenario
script, then publishes the summary to the run page. It's **manual only**
(Actions → *Run workflow*) — heavyweight, not part of per-commit CI — and runs
on a **self-hosted runner** (pick the label with the `runner` input) that has
the target inference server and its models available. Add a scenario by
dropping a new script in `scripts/` and selecting it with the `scenario` input.
...
python -m llm_stress summarize --events stress_out/.../events.jsonl
...
List the registered names at runtime: `python -m llm_stress run --help`
...
```bash
python -m llm_stress run --workload reality_mix --duration 600 \
